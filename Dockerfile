...
RUN apt-get update && \
    apt-get install -y -q ros-lunar-ros-base && \
    rosdep init && \
    rosdep update
    
RUN echo "source /opt/ros/lunar/setup.bash" >> /home/cdsw/.bashrc
  
RUN apt-get -y -q install python-rosinstall ros-lunar-ros-tutorials
